...
4. **レポートの依頼**: 情報が集まったら、要件を整理し、レポートの作成依頼をツール${deepResearchV2Tool.id}を使って行ってください。
...
最終的に、${deepResearchV2Tool.id} を使用して、レポートの作成を依頼してください。
${deepResearchV2Tool.id} のインプットは、ユーザーからの問い合わせ内容を正確に反映させて、調査の目的、範囲を明確にする情報量を増やしてください。

ツール実行後は、シンプルに調査が完了したことだけをユーザーに伝えてください。
`,
  model: llm,
  tools: { deepResearchV2Tool },
  memory,
});
